...
class CustomEditTextPassword : AppCompatEditText, View.OnTouchListener {
    ...
    private fun init() {
        ...
        addTextChangedListener(object : TextWatcher {
            ...
            override fun onTextChanged(s: CharSequence, start: Int, before: Int, count: Int) {
                when{
                    s.toString().isEmpty() -> error = resources.getString(R.string.kolom_tidak_boleh_kosong)

                    s.length < 8 -> error = resources.getString(R.string.password_kurang_dari_8_karakter)
                }
            }
